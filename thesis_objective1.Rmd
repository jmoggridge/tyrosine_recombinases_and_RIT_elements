...
The domains data have been collected in R as follows:
...
write_rds(ref_domains, './data/SMART_domains.rds')
glimpse(ref_domains)
```

I compiled all the full AA sequences for the integrases from SMART
...
ref_integrases <- 
  ...
  left_join(read_csv('./data/SMART/smart_subfamily_ids.csv') |> 
            ...
            by = 'subfamily') |> 
  select(subfamily, id, everything())
write_rds(ref_integrases, './data/SMART_integrases.rds')
...
glimpse(SMART_db)
write_rds(SMART_db, './data/SMART_db_full.rds')
...
domains <- read_rds('./data/SMART_domains.rds')
domains <- domains |> 
  mutate(aligned = map2(.x = subfamily, .y = path,
...
Similar to hmmscan but searches for a single domain subfamily at a time, results might be easier to rectangle in R than the hmmscan tables.

Do a search for each subfamily hmm on the whole set of SMART integrases.
```
mkdir hmmsearch_res
