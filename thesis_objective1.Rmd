...
Remove duplicated sequences from the dataframe (leaving them in the fasta is ok for now). Tidy up the protein names (title) to separate accession number from the descriptions.

```{r message=FALSE, warning=FALSE}
# count total number of seqs
n <- non_integrases |> select(-name, -fasta) |> unnest(df) |> nrow()
n
# any duplicated records? -> none
non_integrases |> select(-name, -fasta) |> unnest(df) |>
  duplicated() |> any()
...
# Keep 22250 unique sequences of 22535 in dataframe; keeping first occurrences.
non_integrases <- non_integrases |> 
  select(-fasta) |> 
  unnest(df) |>
  group_by(seq) |> 
  sample_n(1) |> 
  ...
  nest(df = c(title, seq)) |> 
  left_join(non_integrases |> select(name, fasta),
            by = 'name')
...
```
```{r message=FALSE, warning=FALSE}
non_integrases <- read_rds('./data/non_integrases_df.rds') 
glimpse(non_integrases)
```
