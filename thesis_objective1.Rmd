...
I collected all the domain reference sequences from SMART integrase subfamilies into an R dataframe:
...
ref_domains <- 
  ...
  mutate(
    subfamily = str_remove_all(dom_path,
                               './data/SMART/domain_fasta/|\\.fasta'),
...
I compiled all the full AA sequences for the reference integrases from SMART (*i.e.* the full sequences containing the domain sequences above).  
...
glimpse(ref_integrases)
```

----

Finally, I joined the domains and full sequence files into a single huge dataset for all the integrases from SMART.
...
I aligned the domain AA sequences for each subfamily with the decipher package (with default settings), as follows:
...
I installed HMMER (v3.3.2) through Conda and created HMMs for each 'phage integrase' domain sub-family using the multiple alignments. I now have a database of domain models that I can use to annotate the SMART domains in any query protein.
...
I parsed all the `hmmsearch` results tables and rectangled the data by joining bitscores to protein names & their true subfamilies from the SMART_db.rds dataframe.  
...
\  
\  
I plotted the score profiles for each subfamily to explore the discriminative power of the bit-scores. This shows the distribution of scores for a set of sequences from a given subfamily (panel) against each subfamily hmm (y-axis), in terms of the bit scores (x-axis).  
...
# initial_split(hmmsearches)
# 
# set up resampling scheme
# vfold_cv(hmmsearches, v = 5, strata = subfamily)
