...
other_seqs <- 
  map(queries, 
      ~get_Efasta(.x, db = 'protein', apikey, verbose = F,
                  fetch_max = 1000)) |>
...
write_rds(other_seqs, './data/non_integrases_df.rds', compress = 'gz')
...
Write non-integrases to fasta for HMM scoring later on.

```{r}
other_seqs <- read_rds('./data/non_integrases_df.rds') 
glimpse(other_seqs)

# check for any duplicate records -> none
other_seqs |>  select(-name, -fasta) |>  unnest(df) |>
  duplicated() |>  any()
# protein names (title) are all unique
other_seqs |> select(-name, -fasta) |> unnest(df) |>
...
# Keep 22250 unique sequences of 22535 in dataframe
other_seqs |> 
  select(-fasta) |> 
  unnest(df) |>
  group_by(seq) |> 
  sample_n(1) |> 
  nest(df = c(title, seq)) |> 
  left_join(other_seqs |> select(name, fasta), by = 'name')

# check for right number of seqs
other_seqs |> select(-name, -fasta) |> unnest(df) |>
  pull(seq) |> unique() |> length()
# string sets still contain the 
other_seqs |> pull(fasta) |> map_int(length) |> sum()

# combine stringsets to write a fasta file
other_seqs |> 
  pull(fasta) |> 
  reduce(c) |> 
...
### Collect non-integrase sequences for negative classifion
...
#### Get variety of random protein sequences to train & assess 'other' category for non-integrases.

human proteome: https://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/reference_proteomes/Eukaryota/UP000005640/UP000005640_9606.fasta.gz
