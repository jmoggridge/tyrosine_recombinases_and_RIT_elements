...
I installed HMMER through conda using:  
```
conda install -c bioconda hmmer
```


HMMs are built with HMMER `hmmbuild` from multiple sequence alignments
```
hmmbuild subfam1.hmm subfam1.aln
```

To do all the alignments, I ran hmmbuild on the aligned fastas from decipher (within the `./data/SMART/` folder):
```
mkdir ./domain_hmm
